...
export default function Home() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/sheets/read', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ operation: 'read', sheetName: 'budget'  }) });
  ...
  const getHalfwayPoint = (projected) => {
    return projected / 2;
  };

  const handleGoToNewExpenses = () => {
    router.push('/'); // Navigate to the report page
  };
  ...
  //       body: JSON.stringify({ operation: 'create', values: [newItem.item, newItem.projected, newItem.actual], sheetName: 'budget' }),
  //     });
  //     if (!response.ok) throw new Error(`Error: ${response.statusText}`);
  //     await fetchData();
  ...
  //   try {
  //     const range = `budget!A${editItem.index + 1}:C${editItem.index + 1}`;
  ...
  //   try {
  //     const range = `budget!A${index + 2}:C${index + 2}`;
      ...
      <table className="min-w-full bg-white border border-gray-200">
        <thead>
          <tr className="w-full bg-gray-100 border-b">
            ...
            <th className="py-2 px-4 text-left text-gray-600">Remaining Budget</th>
            <th className="py-2 px-4 text-left text-gray-600">Budget Meter</th>
          </tr>
        </thead>
  
        <tbody>
          {data.map((row, index) => {
            const projected = parseFloat(row[1]) || 0;
            const actual = parseFloat(row[2]) || 0;
            const remainingBudget = parseFloat(row[3]) || 0;
            // const remainingBudget = projected - actual;
            

           // Determine class based on actual value
           const meterClass = remainingBudget < 0 ? 'low' : 'high';
            ...
            return (
              <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                ...
                <td className="py-2 px-4 border-b">{row[3]|| 'N/A'}</td>
                <td className="py-2 px-4 border-b">
                <div className={`meter-container ${meterClass}`}>
                    <meter
